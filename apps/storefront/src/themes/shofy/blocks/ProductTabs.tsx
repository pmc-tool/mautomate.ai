...
/* ------------------------------------------------------------------ */
/* Shofy renderer for the product_tabs CMS block ("product area" /      */
/* "Trending Products" in index.html).                                  */
/*                                                                      */
...
/* the SAME code the editor bridge uses) then hands the resolved         */
/* per-tab `groups` to the pure ProductTabsView, which owns the Shofy    */
/* .tp-product-area markup. Splitting the presentational half out lets   */
/* the visual-editor canvas render the IDENTICAL Shofy markup from       */
/* client-fetched groups — WYSIWYG parity. A tab whose category /        */
/* collection / product no longer exists resolves to an empty group and  */
/* is dropped by the View (dangling-ref safe). `countryCode` resolves    */
/* region-scoped prices; we fall back to the default region when absent. */
/* ------------------------------------------------------------------ */
...
const ProductTabs = async (props: ProductTabsData) => {
  ...
  // Fetch every configured tab's live products in parallel; a tab whose
  // binding no longer resolves simply yields an empty group.
