...
/* ------------------------------------------------------------------ */
/* Bazaro renderer for the rich_text CMS block.                         */
/*                                                                      */
...
/* plain contained column (Bazaro's Satoshi body styles apply).         */
/*                                                                      */
/* FAQ accordion variant: when the (open) prop bag additionally carries */
/* an `items` array of question/answer pairs we render a Bootstrap      */
/* accordion (bootstrap.css IS in the Bazaro stylesheet stack) —        */
/* Bootstrap's collapse JS is NOT loaded; expand/collapse is            */
...
/** Sanitize an answer; wrap plain text in a <p> so body styles apply. */
...
const RichText = (props: RichTextData) => {
  ...
  if (faqItems.length) {
    const accordion = (
      <div className="accordion bazaro-faq-accordion">
        {faqItems.map((item, i) => {
    ...
    return (
      <div className="bazaro-rich-text pt-60 pb-60">
