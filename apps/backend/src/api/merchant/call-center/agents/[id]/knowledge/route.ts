...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    // Embed the new entry into the RAG store (best-effort; never
    // block the create on an embedding/provider hiccup).
    try {
      await ingestKnowledgeEntry(cc as any, knowledge)
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error("[call-center] knowledge ingest failed (non-blocking):", e)
    }
