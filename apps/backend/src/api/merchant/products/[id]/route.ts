...
export const PUT = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const {
    title, handle, description, status, prices, inventory_quantity, sku, tags, collection_ids, category_ids, variants: inputVariants,
  } = parsed.data
  ...
  // tags are tagged with metadata.tenant_id so the tenant-scoped filter sees them.
  ...
  if (category_ids !== undefined) {
    productUpdate.categories = category_ids.length ? category_ids.map((id) => ({ id })) : []
  }
  ...
  if (inputVariants && inputVariants.length) {
  ...
  } else if (variant && (prices !== undefined || inventory_quantity !== undefined || sku !== undefined)) {
    const variantUpdate: any = { id: variant.id }
    if (sku !== undefined) variantUpdate.sku = sku
    if (inventory_quantity !== undefined) {
      variantUpdate.manage_inventory = true
      pendingStock.push({ variantId: variant.id, qty: inventory_quantity })
    }
    if (prices !== undefined) {
  ...
  // Apply REAL stock levels at the tenant's default location (replaces the old
  // variant.metadata.inventory_quantity write). Skip gracefully if no location.
