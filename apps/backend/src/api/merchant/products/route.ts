...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const { data } = await query.graph({
    ...
    fields: ["id", "title", "status", "thumbnail", "handle", "created_at", "updated_at", "variants.id", "variants.metadata"],
  ...
  // REAL stock from inventory levels (available per variant). Falls back to the
  // legacy metadata.inventory_quantity only for variants with no inventory item.
  const availableByVariant = await getAvailableByVariant(req, allVariantIds)
  ...
  const products = variantsByProduct.map(({ product: p, variants }) => {
    ...
    let stock = 0
    if (firstVariant) {
      if (firstVariant.id in availableByVariant) {
        stock = availableByVariant[firstVariant.id]
      } else {
        stock = firstVariant?.metadata?.inventory_quantity ?? 0
      }
    }
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const {
    title, handle, description, status, prices, inventory_quantity, sku, tags, collection_ids, category_ids,
    options: inputOptions, variants: inputVariants,
  } = parsed.data

  const productModule: any = req.scope.resolve(Modules.PRODUCT)

  // Resolve tag IDs from tag values (create missing ones). Implicitly-created
  // tags are tagged with metadata.tenant_id so they show up under the
  // tenant-scoped /merchant/product-tags filter (P1 follow-up).
  const tagIds: string[] = []
  if (tags.length) {
  ...
  // Build options/variants payload for Medusa. manage_inventory is forced true so
  // Medusa creates a REAL inventory item per variant; the initial stock is set as
  // an inventory LEVEL below (not variant.metadata.inventory_quantity).
  let workflowOptions: any[] = []
  let workflowVariants: any[] = []
  // Desired initial stock keyed for post-create matching.
  const desiredBySku = new Map<string, number>()
  const desiredByTitle = new Map<string, number>()

  if (inputOptions.length && inputVariants.length) {
    workflowOptions = inputOptions.map((o) => ({
      title: o.title,
      values: o.values,
    }))

    workflowVariants = inputVariants.map((v) => {
      if (v.sku) desiredBySku.set(v.sku, v.inventory_quantity)
      desiredByTitle.set(v.title, v.inventory_quantity)
      return {
        title: v.title,
        sku: v.sku,
        prices: v.prices.length ? v.prices : prices,
        options: v.options,
    ...
    if (sku) desiredBySku.set(sku, inventory_quantity)
    desiredByTitle.set("Default", inventory_quantity)
  ...
  // Every product needs a shipping profile or checkout fails ("shipping profiles
  // not satisfied"). Assign the shared default profile at creation.
  const __q: any = req.scope.resolve("query")
  const { data: __sp } = await __q.graph({
    entity: "shipping_profile",
    filters: { type: "default" } as any,
    fields: ["id"],
    pagination: { take: 1, skip: 0 } as any,
  })
  const __defaultProfileId: string | undefined = __sp?.[0]?.id
      ...
      products: [
        {
          ...
          categories: category_ids.length ? category_ids.map((id) => ({ id })) : undefined,
          sales_channels: [{ id: scId }],
          shipping_profile_id: __defaultProfileId,
  ...
  // stock location for each created variant. Gracefully skip if the tenant has
