...
/**
 * GET /merchant/product-types
 *
 * List product types for this tenant. Types are global in Medusa; scope to this
 * tenant via metadata.tenant_id. Fail-closed: rows without a matching tenant_id
 * (including legacy untagged types) are invisible to this merchant.
 *
...
export const GET = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  try {
    ...
    res.json({
      types: owned.map((t: any) => ({ id: t.id, value: t.value })),
      count: owned.length,
    })
  } catch (e: any) {
    res.status(500).json({ message: e?.message ?? "Failed to load product types" })
  }
}
