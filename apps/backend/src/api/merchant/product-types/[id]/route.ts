...
async function ownedType(req: MedusaRequest, tenantId: string, id: string) {
...
async function inUseCount(
  ...
  return (data || []).filter((p: any) => p.type_id === typeId).length
}
...
const update = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const { id } = req.params
  const existing = await ownedType(req, ctx.tenant.id, id)
  if (!existing) return res.status(404).json({ message: "product type not found" })
  ...
  if (used > 0) {
    return res.status(409).json({
      message: `Cannot delete: ${used} product${
        used === 1 ? "" : "s"
      } still use this type. Reassign them first.`,
    })
  }

  await deleteProductTypesWorkflow(req.scope).run({ input: { ids: [id] } })

  res.json({ id, object: "product_type", deleted: true })
}
