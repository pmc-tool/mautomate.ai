...
exports.validateCustomerAccountCreation = (0, workflows_sdk_1.createStep)(exports.validateCustomerAccountCreationStepId, async (input, { container }) => {
    ...
    let existingCustomers = await customerService.listCustomers({ email });
    // PATCH(mautomate multi-tenant): scope the duplicate check to the SAME
    // STORE. The platform's stamp middleware injects metadata.tenant_id into
    // the customer data, and customer-email uniqueness is per store (see
    // migration-scripts/customer-freedom.sql). Rows stamped for a DIFFERENT
    // tenant must not block this store's registration. Unstamped (legacy /
    // single-tenant) rows keep blocking — byte-for-byte the original
    // behavior when no tenant stamp is present.
    const __tenantId = input.customerData?.metadata?.tenant_id;
    if (__tenantId) {
        existingCustomers = (existingCustomers || []).filter((c) => {
            const owner = c?.metadata?.tenant_id;
            return !owner || owner === __tenantId;
        });
    }
