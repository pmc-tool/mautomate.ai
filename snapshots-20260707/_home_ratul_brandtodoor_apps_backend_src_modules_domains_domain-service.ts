...
const findDomainRow = async (
  ...
  const rows = await domainsModule.listDomains({
    tenant_id: tenantId,
...
export const buyDomain = async (
  ...
  try {
    ...
    let domain: any
    if (existing) {
      const [d] = await domainsModule.updateDomains([
        { id: existing.id, ...domainPayload },
      ] as any)
      domain = d
    } else {
      const [d] = await domainsModule.createDomains([domainPayload] as any)
      domain = d
    }
...
export const getTransferOut = async (
  ...
  try {
    ...
    if (resolved.row) {
      await domainsModule.updateDomains([
...
export const syncDomain = async (
  ...
  try {
    ...
    let domain: any
    if (existing) {
      const [row] = await domainsModule.updateDomains([
        { id: existing.id, ...payload },
      ] as any)
      domain = row
    } else {
      const [row] = await domainsModule.createDomains([
        {
          tenant_id: tenantId,
          domain_name: i.domainName,
          tld,
