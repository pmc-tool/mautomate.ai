      ...
      fetch("/api/theme-products?ids=" + encodeURIComponent(wishIds.join(",")) + "&country=" + countryFromPath())
        ...
        .then(function (d) {
          ...
          wishGrid.innerHTML = products.map(function (p) {
            ...
            return '<article class="sf-card" data-wishlist-card>' +
              '<div class="sf-card-media"><a href="' + href + '" class="sf-card-img">' +
              (img ? '<img class="sf-card-img-main" src="' + escapeHtml(img) + '" alt="' + escapeHtml(p.title) + '" loading="lazy">' : '<span class="sf-card-img-ph"></span>') +
              "</a></div>" +
              '<div class="sf-card-body">' +
              '<h3 class="sf-card-title"><a href="' + href + '">' + escapeHtml(p.title) + "</a></h3>" +
              '<div class="sf-card-price"><span class="sf-price-now">' + escapeHtml(p.price_formatted || "") + "</span></div>" +
              '<button type="button" class="sf-wish-btn sf-wish-btn-sm" data-wishlist-toggle data-product-id="' + escapeHtml(p.id) + '"><span class="sf-wish-heart" aria-hidden="true"><i class="fas fa-heart"></i></span> <span data-wish-label>Saved to wishlist</span></button>' +
              "</div></article>";
          }).join("");
