/* Kidverz (Liquid) theme — client behaviour. No network calls except cart. */
(function () {
  "use strict";

  /* ---- hero slider ---- */
  document.querySelectorAll("[data-hero]").forEach(function (hero) {
    var slides = Array.prototype.slice.call(hero.querySelectorAll(".kv-hero-slide"));
  ...
  document.querySelectorAll("[data-pdp-thumbs]").forEach(function (strip) {
    ...
    if (!main) return;
    strip.querySelectorAll(".kv-pdp-thumb").forEach(function (thumb) {
      thumb.addEventListener("click", function () {
        var full = thumb.getAttribute("data-full");
        if (full) main.setAttribute("src", full);
        strip.querySelectorAll(".kv-pdp-thumb").forEach(function (t) { t.classList.toggle("is-active", t === thumb); });
      });
    });
  });
  ...
  function setCartCount(n) {
    var link = document.querySelector(".kv-icon-cart");
    if (!link) return;
    var badge = link.querySelector(".kv-cart-count");
    if (!badge) { badge = document.createElement("span"); badge.className = "kv-cart-count"; link.appendChild(badge); }
    badge.textContent = n;
  }
      ...
      fetch("/api/theme-cart", {
        ...
        .then(function (res) {
          ...
          if (res.ok) {
            ...
            if (msg) { msg.hidden = false; msg.textContent = "Added to your cart. Yay!"; }
