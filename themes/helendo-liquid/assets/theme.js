/* Helendo (Liquid) theme — client behaviour. No network calls except cart. */
(function () {
  "use strict";

  /* ---- hero slider ---- */
  document.querySelectorAll("[data-hero]").forEach(function (hero) {
    var slides = Array.prototype.slice.call(hero.querySelectorAll(".hl-hero-slide"));
  ...
  /* ---- offcanvas menu (the Helendo header's only navigation) ---- */
  ...
  if (toggle && offcanvas) {
    ...
    toggle.addEventListener("click", function (e) {
      e.preventDefault();
      setOpen(!offcanvas.classList.contains("is-open"));
    });
    offcanvas.addEventListener("click", function (e) {
      if (e.target === offcanvas) setOpen(false);
    });
    var close = offcanvas.querySelector("[data-offcanvas-close]");
    if (close) close.addEventListener("click", function () { setOpen(false); });
    document.addEventListener("keydown", function (e) {
      if (e.key === "Escape" && offcanvas.classList.contains("is-open")) setOpen(false);
    });
  }
  ...
  document.querySelectorAll("[data-pdp-thumbs]").forEach(function (strip) {
    ...
    if (!main) return;
    strip.querySelectorAll(".hl-pdp-thumb").forEach(function (thumb) {
      thumb.addEventListener("click", function () {
        var full = thumb.getAttribute("data-full");
        if (full) main.setAttribute("src", full);
        strip.querySelectorAll(".hl-pdp-thumb").forEach(function (t) { t.classList.toggle("is-active", t === thumb); });
      });
    });
  });
  ...
  function setCartCount(n) {
    var link = document.querySelector(".hl-cart");
    if (!link) return;
    var badge = link.querySelector(".hl-cart-count");
    if (!badge) { badge = document.createElement("span"); badge.className = "hl-cart-count"; link.appendChild(badge); }
    badge.textContent = n;
  }
      ...
      fetch("/api/theme-products?ids=" + encodeURIComponent(wishIds.join(",")) + "&country=" + countryFromPath())
        ...
        .then(function (d) {
          ...
          wishGrid.innerHTML = products.map(function (p) {
            ...
            return '<article class="hl-card" data-wishlist-card>' +
              '<div class="hl-card-media"><a href="' + href + '" class="hl-card-img">' +
              (img ? '<img class="hl-card-img-main" src="' + escapeHtml(img) + '" alt="' + escapeHtml(p.title) + '" loading="lazy">' : "") +
              "</a></div>" +
              '<div class="hl-card-body">' +
              '<h6 class="hl-card-title"><a href="' + href + '">' + escapeHtml(p.title) + "</a></h6>" +
              '<div class="hl-card-price"><span class="hl-price-now">' + escapeHtml(p.price_formatted || "") + "</span></div>" +
              '<button type="button" class="hl-wish-btn hl-wish-btn-sm" data-wishlist-toggle data-product-id="' + escapeHtml(p.id) + '"><span class="hl-wish-heart" aria-hidden="true">&hearts;</span> <span data-wish-label>Saved to wishlist</span></button>' +
              "</div></article>";
          }).join("");
