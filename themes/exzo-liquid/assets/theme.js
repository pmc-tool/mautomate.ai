/* Exzo (Liquid) theme — client behaviour. No network calls except cart. */
(function () {
  "use strict";

  /* ---- hero slider ---- */
  document.querySelectorAll("[data-hero]").forEach(function (hero) {
    var slides = Array.prototype.slice.call(hero.querySelectorAll(".ex-hero-slide"));
  ...
  document.querySelectorAll("[data-pdp-thumbs]").forEach(function (strip) {
    ...
    if (!main) return;
    strip.querySelectorAll(".ex-pdp-thumb").forEach(function (thumb) {
      thumb.addEventListener("click", function () {
        var full = thumb.getAttribute("data-full");
        if (full) main.setAttribute("src", full);
        strip.querySelectorAll(".ex-pdp-thumb").forEach(function (t) { t.classList.toggle("is-active", t === thumb); });
      });
    });
  });
  ...
  function setCartCount(n) {
    var link = document.querySelector(".ex-cart");
    if (!link) return;
    var badge = link.querySelector(".ex-cart-count");
    if (!badge) { badge = document.createElement("span"); badge.className = "ex-cart-count"; link.appendChild(badge); }
    badge.textContent = n;
  }
      ...
      fetch("/api/theme-cart", {
        ...
        .then(function (res) {
          ...
          if (res.ok) {
            ...
            if (msg) { msg.hidden = false; msg.textContent = "Added to your bag."; }
    ...
    var cartMutate = function (payload, msgEl) {
      ...
      fetch("/api/theme-cart", {
        ...
        .then(function (res) {
          ...
          showCartMsg(msgEl, (res.d && res.d.error) || "Could not update your bag.");
        })
        .catch(function () {
          cartBusy = false;
          cartPage.classList.remove("is-busy");
          showCartMsg(msgEl, "Could not update your bag.");
        });
    };
      ...
      fetch("/api/theme-products?ids=" + encodeURIComponent(wishIds.join(",")) + "&country=" + countryFromPath())
        ...
        .then(function (d) {
          ...
          wishGrid.innerHTML = products.map(function (p) {
            ...
            return '<article class="ex-card" data-wishlist-card>' +
              '<div class="ex-card-media"><a href="' + href + '" class="ex-card-img">' +
              (img ? '<img class="ex-card-img-main" src="' + escapeHtml(img) + '" alt="' + escapeHtml(p.title) + '" loading="lazy">' : "") +
              "</a>" +
              '<div class="ex-card-actions"><a class="ex-btn ex-btn-sm" href="' + href + '">View product</a></div></div>' +
              '<div class="ex-card-body">' +
              '<h3 class="ex-card-title"><a href="' + href + '">' + escapeHtml(p.title) + "</a></h3>" +
              '<div class="ex-card-price"><span class="ex-price-now">' + escapeHtml(p.price_formatted || "") + "</span></div>" +
              '<button type="button" class="ex-wish-btn ex-wish-btn-sm" data-wishlist-toggle data-product-id="' + escapeHtml(p.id) + '"><span class="ex-wish-heart" aria-hidden="true">&hearts;</span> <span data-wish-label>Saved to wishlist</span></button>' +
              "</div></article>";
          }).join("");
