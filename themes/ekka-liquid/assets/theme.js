      ...
      fetch("/api/theme-products?ids=" + encodeURIComponent(wishIds.join(",")) + "&country=" + countryFromPath())
        ...
        .then(function (d) {
          ...
          wishGrid.innerHTML = products.map(function (p) {
            ...
            return '<article class="ek-card" data-wishlist-card>' +
              '<div class="ek-card-media"><a href="' + href + '" class="ek-card-img">' +
              (img ? '<img class="ek-card-img-main" src="' + escapeHtml(img) + '" alt="' + escapeHtml(p.title) + '" loading="lazy">' : "") +
              "</a></div>" +
              '<div class="ek-card-body">' +
              '<h3 class="ek-card-title"><a href="' + href + '">' + escapeHtml(p.title) + "</a></h3>" +
              '<div class="ek-card-price"><span class="ek-price-new">' + escapeHtml(p.price_formatted || "") + "</span></div>" +
              '<button type="button" class="ek-wish-btn ek-wish-btn-sm" data-wishlist-toggle data-product-id="' + escapeHtml(p.id) + '"><i class="far fa-heart ek-wish-heart" aria-hidden="true"></i> <span data-wish-label>Saved to wishlist</span></button>' +
              "</div></article>";
          }).join("");
