...
(function () {
  ...
  document.querySelectorAll("[data-hero]").forEach(function (hero) {
    var slides = Array.prototype.slice.call(hero.querySelectorAll(".lz-hero-slide"));
  ...
  document.querySelectorAll("[data-pdp-thumbs]").forEach(function (strip) {
    ...
    if (!main) return;
    strip.querySelectorAll(".lz-pdp-thumb").forEach(function (thumb) {
      thumb.addEventListener("click", function () {
        var full = thumb.getAttribute("data-full");
        if (full) main.setAttribute("src", full);
        strip.querySelectorAll(".lz-pdp-thumb").forEach(function (t) { t.classList.toggle("is-active", t === thumb); });
      });
    });
  });
  ...
  function setCartCount(n) {
    var link = document.querySelector(".lz-cart");
    if (!link) return;
    var badge = link.querySelector(".lz-cart-count");
    if (!badge) { badge = document.createElement("span"); badge.className = "lz-cart-count"; link.appendChild(badge); }
    badge.textContent = n;
  }
      ...
      fetch("/api/theme-products?ids=" + encodeURIComponent(wishIds.join(",")) + "&country=" + countryFromPath())
        ...
        .then(function (d) {
          ...
          wishGrid.innerHTML = products.map(function (p) {
            ...
            return '<article class="lz-card" data-wishlist-card>' +
              '<div class="lz-card-media"><a href="' + href + '" class="lz-card-img">' +
              (img ? '<img class="lz-card-img-main" src="' + escapeHtml(img) + '" alt="' + escapeHtml(p.title) + '" loading="lazy">' : "") +
              "</a></div>" +
              '<div class="lz-card-body">' +
              '<h3 class="lz-card-title"><a href="' + href + '">' + escapeHtml(p.title) + "</a></h3>" +
              '<div class="lz-card-price"><span class="lz-price-now">' + escapeHtml(p.price_formatted || "") + "</span></div>" +
              '<button type="button" class="lz-wish-btn lz-wish-btn-sm" data-wishlist-toggle data-product-id="' + escapeHtml(p.id) + '"><span class="lz-wish-heart" aria-hidden="true">&hearts;</span> <span data-wish-label>Saved to wishlist</span></button>' +
              "</div></article>";
          }).join("");
