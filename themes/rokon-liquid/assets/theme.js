/* Rokon (Liquid) theme — client behaviour. No network calls except cart. */
(function () {
  "use strict";

  /* ---- hero slider ---- */
  document.querySelectorAll("[data-hero]").forEach(function (hero) {
    var slides = Array.prototype.slice.call(hero.querySelectorAll(".rk-hero-slide"));
  ...
  document.querySelectorAll("[data-pdp-thumbs]").forEach(function (strip) {
    ...
    if (!main) return;
    strip.querySelectorAll(".rk-pdp-thumb").forEach(function (thumb) {
      thumb.addEventListener("click", function () {
        var full = thumb.getAttribute("data-full");
        if (full) main.setAttribute("src", full);
        strip.querySelectorAll(".rk-pdp-thumb").forEach(function (t) { t.classList.toggle("is-active", t === thumb); });
      });
    });
  });
  ...
  function setCartCount(n) {
    var link = document.querySelector(".rk-cart");
    if (!link) return;
    var badge = link.querySelector(".rk-cart-count");
    if (!badge) { badge = document.createElement("span"); badge.className = "rk-cart-count"; link.appendChild(badge); }
    badge.textContent = n;
  }
  ...
  /* ---- cart page: quantity / remove / promo code. Mutations post to the
     ...
     server re-renders the Liquid cart with authoritative totals. Elements
     are selected only via data attributes (theme-agnostic contract). ---- */
...
/* ==========================================================================
   ACCOUNT + WISHLIST — behavioural contract shared with the reference
   implementation (learts-liquid). Forms post JSON to the same-origin
   /api/theme-account bridge (the SAME server actions the React pages use —
   session, tenant binding and validation are identical). Wishlist ids live
   in localStorage ff_wishlist (platform-wide contract) and resolve to cards
   via /api/theme-products. All selection is via data attributes; only the
   rendered wishlist-card HTML is Rokon-specific.
   ========================================================================== */
(function () {
  "use strict";
  ...
  /* ---- account pages: auth + profile + addresses ---- */
      ...
      fetch("/api/theme-products?ids=" + encodeURIComponent(wishIds.join(",")) + "&country=" + countryFromPath())
        ...
        .then(function (d) {
          ...
          wishGrid.innerHTML = products.map(function (p) {
            ...
            return '<article class="rk-card" data-wishlist-card>' +
              '<div class="rk-card-media"><a href="' + href + '" class="rk-card-img">' +
              (img ? '<img class="rk-card-img-main" src="' + escapeHtml(img) + '" alt="' + escapeHtml(p.title) + '" loading="lazy">' : "") +
              "</a></div>" +
              '<div class="rk-card-body">' +
              '<h3 class="rk-card-title"><a href="' + href + '">' + escapeHtml(p.title) + "</a></h3>" +
              '<div class="rk-card-price"><span class="rk-price-now">' + escapeHtml(p.price_formatted || "") + "</span></div>" +
              '<button type="button" class="rk-wish-btn rk-wish-btn-sm" data-wishlist-toggle data-product-id="' + escapeHtml(p.id) + '"><span class="rk-wish-heart" aria-hidden="true">&hearts;</span> <span data-wish-label>Saved to wishlist</span></button>' +
              "</div></article>";
          }).join("");
