...
export type CollectionDetail = ProductCollection & {
  products: { id: string; title: string; status: string; thumbnail?: string | null }[]
}
...
export async function getCollection(
  ...
  return request<{ collection: CollectionDetail }>(`/merchant/collections/${id}`, { token })
}
...
export async function updateCollectionProducts(
  ...
  return request<{ collection: CollectionDetail }>(`/merchant/collections/${id}`, {
    method: "PUT",
    token,
    body,
  })
...
export async function getDiscount(token: string, id: string): Promise<{ discount: Discount }> {
...
export async function deleteDiscount(token: string, id: string): Promise<void> {
  await request<void>(`/merchant/discounts/${id}`, { method: "DELETE", token })
}
